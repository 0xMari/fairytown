...
const mossGeometry = new THREE.SphereGeometry(1, 12, 8);
...
function createMossTransforms({
  ...
  const transforms = [];
  const lodDensity = THREE.MathUtils.clamp(lodFactor, 0.35, 1.8);
  const lodAlpha = smoothstep01(0.35, 1.8, lodDensity);
  const lodStepScale = THREE.MathUtils.lerp(1.58, 0.82, lodAlpha);
  const lodPresenceScale = THREE.MathUtils.lerp(0.45, 1.32, lodAlpha);
  const layers = [
    {
      step: 1.55,
      jitter: 0.64,
